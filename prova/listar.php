...
$filtro      = isset($_GET['tipo']) ? $_GET['tipo'] : 'todas';
$qtd_multipla = 0; $qtd_texto = 0;
foreach ($perguntas as $p) {
    if ($p['tipo'] === 'multipla') $qtd_multipla++;
    elseif ($p['tipo'] === 'texto') $qtd_texto++;
}

if ($filtro === 'multipla') {
    $lista = array();
    foreach ($perguntas as $p) { if ($p['tipo'] === 'multipla') $lista[] = $p; }
} elseif ($filtro === 'texto') {
    $lista = array();
    foreach ($perguntas as $p) { if ($p['tipo'] === 'texto') $lista[] = $p; }
} else {
    $lista = $perguntas;
}

$sucesso = isset($_GET['excluido']) ? $_GET['excluido'] : '';
...
<div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;flex-wrap:wrap">
  ...
  <a href="listar.php?tipo=multipla" class="btn btn-sm <?= $filtro==='multipla'?'btn-primary':'btn-secondary' ?>">Múltipla (<?= $qtd_multipla ?>)</a>
  <a href="listar.php?tipo=texto" class="btn btn-sm <?= $filtro==='texto'?'btn-primary':'btn-secondary' ?>">Dissertativa (<?= $qtd_texto ?>)</a>
